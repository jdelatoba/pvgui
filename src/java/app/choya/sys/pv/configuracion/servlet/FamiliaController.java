...
import app.choya.sys.pv.configuracion.Familia;
import app.choya.sys.pv.configuracion.FamiliaBean;
import app.choya.sys.pv.configuracion.FamiliaI;
 ...
 * @author jdelatoba
 */
@WebServlet(name = "FamiliaController", urlPatterns = {"/admon/FamiliaController"})
public class FamiliaController extends HttpServlet {
    
    static final Logger LOGGER = Logger.getLogger(FamiliaController.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            FamiliaBean gb = new FamiliaBean();
            
            gb.setId(id);
            gb.setDescripcion(descripcion);
          
            
            FamiliaI grp = new Familia();
            //operacion = "guardar";
            int resultado = 0;
            System.out.println("operaciones Familia "+operacion); 
            switch (operacion) {
                case "actualizar":{
                    try{
                        if(gb.getId() == 0){
                            resultado = grp.setFamilia(gb);
                            
                            out.println("<input type=\"hidden\" id=\"resultado\" name=\"resultado\" value=\""+resultado+"\" />");
                        }else{
                            resultado = grp.updateFamilia(gb);
                            out.println("<input type=\"hidden\" id=\"resultado\" name=\"resultado\" value=\""+resultado+"\" />");
                        }    
                        ...
                        throw new SqlAppsException(ex, FamiliaController.class.getName() + "-- metodo: setImpuesto", LOGGER);
                ...
                case "eliminar_impuesto":
                    ...
                    try{
                        resultado = grp.deleteFamilia(id);
                        out.println("<input type=\"hidden\" id=\"resultado\" name=\"resultado\" value=\""+resultado+"\" />");
                    } catch (SqlAppsException ex) {
                        ...
                        out.println("<input type='text' id='mensaje' name='mensaje' value=\""+ex.getMensaje()+"\" />");
                        out.println("<input type='text' id='codigo_error' name='codigo_error' value='"+ex.getCodigo_error()+"' />");
                        throw new SqlAppsException(ex, FamiliaController.class.getName() + "-- metodo: deleteImpuesto", LOGGER);
                    }
                break;    
                default:
                    break;
